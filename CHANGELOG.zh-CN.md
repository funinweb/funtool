...
- ObjectId类适配浏览器环境，实现跨平台base64转换

  - 优化ObjectId.toBase64方法，兼容浏览器btoa和Node.js Buffer环境
...
- refactor(regex): 重构插件系统并改进类型定义

  - 将插件结构由数组改为对象，支持通过动态属性访问插件，提升调用便利性和类型安全。
  - 更新 `RuleName` 类型定义，改为 `keyof typeof plugins`，使其自动匹配当前所有插件名称，增强类型推导。
  - 在 `Regex` 类中动态将插件方法添加为实例属性，支持链式或点访问插件，简化使用。
  - 优化 `isNumber` 方法，增加类型谓词，提升类型保护准确性。
  - 导出 `parseQuery` 相关方法，增强对查询字符串解析的支持。
  - 代码结构更清晰，易于维护和扩展。
...
- 新增支持链式调用和自定义精度的 `MathX` 数学工具类
  - 引入 `MathX` 类，实现灵活的数学计算，支持自定义精度和舍入模式
  - 添加中英文文档说明及完整测试用例
  - 更新构建配置及国际化文件，支持新增功能
...
- 在rollup.config.js和package.json中添加types字段，明确指定类型声明文件的路径
...
- 新增完整的中英文文档内容
- 配置文档站点的部署设置
- 添加内置 regex 插件的文档（如 email、phone 等）
- 完善 regex 模块的 API 文档
